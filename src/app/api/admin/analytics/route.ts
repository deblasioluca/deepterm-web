...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const [
      ...
      prisma.user.count(),
      prisma.team.count(),
      prisma.team.count({ where: { subscriptionStatus: 'active', plan: { not: 'starter' } } }),
      prisma.team.findMany({ where: { subscriptionStatus: 'active', plan: { not: 'starter' } }, select: { plan: true, seats: true } }),
      prisma.user.count({ where: { createdAt: { gte: startDate } } }),
      prisma.team.count({ where: { createdAt: { gte: startDate } } }),
      prisma.team.groupBy({ by: ['plan'], _count: true }),
      prisma.team.findMany({ take: 5, orderBy: { members: { _count: 'desc' } }, include: { _count: { select: { members: true } } } }),
