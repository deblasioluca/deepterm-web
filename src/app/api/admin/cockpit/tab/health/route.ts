...
async function checkAiDevMac() {
  try {
    const ctrl = new AbortController();
    const tid = setTimeout(() => ctrl.abort(), 3000);
    const res = await fetch(`${process.env.AIRFLOW_URL || "http://192.168.20.222:8080"}/api/v1/health`, {
      signal: ctrl.signal,
    });
    clearTimeout(tid);
    ...
    try {
      const { stdout } = await execAsync(`ping -c 1 -W 2 ${process.env.AI_DEV_MAC_HOST || "192.168.20.222"} 2>/dev/null`);
...
async function checkAirflow() {
  try {
    ...
    const url = settings?.value || process.env.AIRFLOW_URL || "http://192.168.20.222:8080";
    const ctrl = new AbortController();
    const tid = setTimeout(() => ctrl.abort(), 3000);
    ...
    return NextResponse.json({
      health: {
        addresses: {
          ...
          airflow: process.env.AIRFLOW_URL || "http://192.168.20.222:8080",
        },
        pi: {
          status: "online",
