...
export async function POST(req: NextRequest) {
  try {
    ...
    switch (action) {
      ...
      case 'manual-pr':
        updates.status = 'in_progress';
        await logEvent(storyId, 'implement', 'progress', 'Manual PR by operator', 'human');
        break;
      ...
      case 'manual-fix':
        // Record step duration for ETA
        {
          ...
          if (durStory?.lifecycleStartedAt) {
            ...
            await recordStepDuration(storyId, 'implement', dur);
          }
        }
        updates.status = 'in_progress';
        updates.lifecycleStep = 'test';
        updates.lifecycleStartedAt = new Date();
        await logEvent(storyId, 'implement', 'completed', 'Manual fix by operator — advancing to test', 'human');
        await logEvent(storyId, 'test', 'started', 'Test step started after manual fix', 'system');
        break;
